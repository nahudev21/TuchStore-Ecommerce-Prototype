...
export default function AdminEditProduct({ onClose, data, getProducts }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await editProductRequest(formData, data.id);
    if (res.success) {
      const resImage = await uploadProductImagesRequest(
        res.data.id,
        files.files
      );
